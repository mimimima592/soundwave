...
export function FollowersPage() {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      console.error('[FollowersPage] loadMore error:', err);
    } finally {
    ...
    (async () => {
      try {
        ...
        const [userData, followersData] = await Promise.all([
          ...
          scAPI.getUserFollowers(Number(id), INITIAL_LIMIT),
        ]);
        if (cancelled) return;
        setTargetUser(userData);
        setUsers(followersData.collection);
        setNextHref(followersData.next_href);
      } catch (err) {
