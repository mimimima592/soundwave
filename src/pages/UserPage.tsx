...
export function UserPage() {
  ...
  useEffect(() => {
    if (!oauthToken) {
      setError('Требуется авторизация для просмотра профиля');
      setLoading(false);
      return;
    }
    ...
    if (cached) {
      ...
      // Обновляем статус подписки в фоне — кеш мог содержать устаревшее значение
      if (!cached.isOwnProfile) {
        scAPI.checkFollowing(cached.user.id)
          .then((fresh) => { if (!cancelled) setIsFollowing(fresh); })
          .catch(() => {});
      }
    ...
    (async () => {
      try {
        ...
        const [tracksData, playlistsData, likesData, followingCheck] = await Promise.all([
          scAPI.getUserTracks(userData.id, PAGE_SIZE),
          scAPI.getUserPlaylists(userData.id, 50),
          scAPI.getUserLikes(userData.id, PAGE_SIZE),
          // userData.following уже есть в ответе аутентифицированного getUser —
          // используем его напрямую, иначе делаем отдельный запрос checkFollowing
          !ownProfile && meData
            ? (userData.following !== undefined
                ? Promise.resolve(userData.following)
                : scAPI.checkFollowing(userData.id))
            : Promise.resolve(false),
        ]);

        const following = followingCheck as boolean;
  ...
  const handleToggleFollow = async () => {
    ...
    // Optimistic update - сразу меняем состояние
    ...
    <div key={user.id} className="max-w-6xl mx-auto animate-slide-up">
      ...
      {(() => {
        ...
        return (
          <TabBar
            tabs={[
              { id: 'all' as Tab, label: 'Все', count: trackCount + playlistCount + likesCount },
              { id: 'popular' as Tab, label: 'Популярное' },
              { id: 'tracks' as Tab, label: 'Треки', count: trackCount },
              { id: 'playlists' as Tab, label: 'Плейлисты', count: playlistCount },
              { id: 'likes' as Tab, label: 'Лайки', count: likesCount },
            ]}
            active={activeTab}
      ...
      {/* Контент */}
      {totalItems === 0 ? (
        <EmptyState title="Ничего не найдено" description="Здесь пока нет контента" />
      ) : (
        <div key={activeTab} className="space-y-6 animate-slide-up">
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-semibold">
                  {activeTab === 'likes' ? 'Лайки' : activeTab === 'popular' ? 'Популярное' : 'Треки'}
                </h2>
                {activeTab === 'all' && currentTracks.length > 5 && (
                  <button onClick={() => setActiveTab('tracks')} className="text-sm text-accent hover:underline">Смотреть все</button>
                )}
              </div>
          ...
          {activeTab === 'all' && (topTracks.length > 0 || topTracksLoading) && (
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                {topTracks.length > 0 && (
                  <button onClick={() => setActiveTab('popular')} className="text-sm text-accent hover:underline">Смотреть все</button>
                )}
              </div>
              {topTracksLoading ? (
                <div className="space-y-1">
          ...
          {activeTab === 'all' && likes.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-semibold">Лайки</h2>
                {likes.length > 5 && (
                  <button onClick={() => setActiveTab('likes')} className="text-sm text-accent hover:underline">Смотреть все</button>
                )}
              </div>
              <div className="space-y-1">
