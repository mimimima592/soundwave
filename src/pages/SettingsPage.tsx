...
export function SettingsPage() {
  ...
  const setFreezeHoverOnScroll = useUIStore((s) => s.setFreezeHoverOnScroll);
  ...
  const handleClearSession = async () => {
    try {
      ...
      if (success) { setOAuthToken(null); scAPI.setOAuthToken(null); alert('Сессия очищена'); }
      else alert('Ошибка очистки сессии');
    } catch { alert('Ошибка очистки сессии'); }
  };
  ...
  const bgOpts: { id: BackgroundType; label: string; icon: React.ReactNode }[] = [
    { id: 'none', label: 'Нет', icon: <X size={13} /> },
    { id: 'gif', label: 'GIF / URL', icon: <ImageIcon size={13} /> },
    { id: 'artwork', label: 'Артворк', icon: <Disc3 size={13} /> },
    { id: 'color', label: 'Цвет', icon: <Palette size={13} /> },
  ];

  return (
    <div className="max-w-3xl">
      <PageHeader title="Настройки" subtitle="Персонализируй Soundwave под себя" />

      <SectionTitle action={
        <button onClick={() => { setEditorMode('new'); setEditingTheme(themes.find(t => t.id === activeThemeId) ?? themes[0]); }}
      ...
      <SectionTitle>Фон приложения</SectionTitle>
      <SettingsCard>
        <label className="block text-[12px] font-semibold uppercase tracking-[0.1em] mb-3" style={{ color: 'rgb(var(--theme-text-dim) / 0.7)' }}>Тип фона</label>
        ...
        {(bgType === 'gif' || bgType === 'color') && (<>
          ...
          <label className="block text-[12px] font-semibold uppercase tracking-[0.1em] mb-2" style={{ color: 'rgb(var(--theme-text-dim) / 0.7)' }}>
            {bgType === 'gif' ? 'URL изображения / GIF' : 'CSS цвет или градиент'}
          </label>
        ...
        {bgType === 'gif' && recentGifs.length > 0 && (<>
          ...
          <div className="flex items-center justify-between mb-3">
            <label className="text-[11px] font-semibold uppercase tracking-[0.1em]" style={{ color: 'rgb(var(--theme-text-dim)/0.7)' }}>Последние GIF</label>
            <button onClick={clearRecentGifs} className="text-[11.5px] hover:text-accent transition-colors" style={{ color: 'rgb(var(--theme-text-dim))' }}>Очистить</button>
          </div>
        ...
        {bgType !== 'none' && (<>
          ...
          <div className="space-y-4">
            <SliderRow label="Прозрачность" value={bgOpacity} min={0} max={1} step={0.01} format={(v) => `${Math.round(v*100)}%`} onChange={(v) => setBackground({ opacity: v })} />
            {bgType !== 'color' && <SliderRow label="Размытие" value={bgBlur} min={0} max={50} step={1} format={(v) => `${v}px`} onChange={(v) => setBackground({ blur: v })} />}
          </div>
        </>)}
      </SettingsCard>

      <SectionTitle>Авторизация SoundCloud</SectionTitle>
      <SettingsCard>
        {oauthToken ? (
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <div className="text-[12px] mt-1" style={{ color: 'rgb(var(--theme-text-dim))' }}>Лента, плейлисты и лайки доступны</div>
            </div>
          ...
          <div className="space-y-4">
            <p className="text-[13px]" style={{ color: 'rgb(var(--theme-text-dim))' }}>Авторизуйся через SoundCloud — токен будет извлечён автоматически.</p>
            <div>
              <label className="block text-[11px] font-semibold uppercase tracking-[0.1em] mb-2" style={{ color: 'rgb(var(--theme-text-dim)/0.7)' }}>Куки из браузера (JSON, опционально)</label>
            ...
            <button onClick={handleAuth} disabled={isAuthenticating}
              ...
              <LogIn size={15} />{isAuthenticating ? 'Авторизация...' : 'Авторизоваться через SoundCloud'}
            </button>
            <Divider />
            <div>
              <label className="block text-[11px] font-semibold uppercase tracking-[0.1em] mb-2" style={{ color: 'rgb(var(--theme-text-dim)/0.7)' }}>Или вставь токен вручную</label>
              <div className="flex gap-2">
        ...
        {obsWidgetEnabled && (<>
          ...
          {widgetSettingsOpen && (
            <div className="space-y-4 mt-4">
              <div>
                <label className="block text-[11px] font-semibold uppercase tracking-[0.1em] mb-2" style={{ color: 'rgb(var(--theme-text-dim)/0.7)' }}>Ссылка для OBS</label>
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => { navigator.clipboard.writeText('http://127.0.0.1:9988/widget'); setWidgetUrlCopied(true); setTimeout(() => setWidgetUrlCopied(false), 2000); }}
                    ...
                    {widgetUrlCopied ? <><CheckCheck size={13} style={{ color: '#4ade80' }} /> Скопировано</> : <><Copy size={13} /> Копировать</>}
                  </button>
                </div>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[13px]" style={{ color: 'rgb(var(--theme-text-dim))' }}>Акцент цвет</span>
              ...
              <div className="flex items-center justify-between">
                <span className="text-[13px]" style={{ color: 'rgb(var(--theme-text-dim))' }}>Фон</span>
                <div className="flex rounded-xl overflow-hidden text-[12px] font-medium" style={{ border: '1px solid rgb(var(--theme-border)/0.4)' }}>
                  {(['artwork', 'blur'] as const).map(t => (
                    <button key={t} onClick={() => setWidgetBgType(t)} className="px-3 py-1.5 transition-colors"
                      style={widgetBgType === t ? { background: 'rgb(var(--theme-accent))', color: 'rgb(var(--theme-accent-fg))' } : { background: 'rgb(var(--theme-surface-alt))', color: 'rgb(var(--theme-text-dim))' }}>
                      {t === 'artwork' ? 'Арт' : 'Blur OBS'}
                    </button>
                  ))}
                </div>
              </div>
              <SliderRow label="Размытие фона" value={widgetBgBlur} min={0} max={80} step={1} onChange={setWidgetBgBlur} format={v => v + 'px'} />
              <SliderRow label="Прозрачность оверлея" value={widgetOverlayOpacity} min={0.3} max={0.97} step={0.01} onChange={setWidgetOverlayOpacity} format={v => Math.round(v*100) + '%'} />
              <div className="rounded-xl p-3.5 space-y-2 text-[12px]" style={{ background: 'rgb(var(--theme-bg))', color: 'rgb(var(--theme-text-dim))' }}>
                <div className="flex items-center gap-1.5 font-semibold mb-2" style={{ color: 'rgb(var(--theme-text))' }}><Tv2 size={13} /> Как добавить в OBS</div>
                <div>1. Добавь источник <code className="px-1.5 py-0.5 rounded-lg text-[11px]" style={{ background: 'rgb(var(--theme-surface-alt))' }}>Browser</code></div>
                <div>2. Вставь ссылку в поле URL</div>
                <div>3. Ширина <code className="px-1.5 py-0.5 rounded-lg text-[11px]" style={{ background: 'rgb(var(--theme-surface-alt))' }}>640</code>, высота <code className="px-1.5 py-0.5 rounded-lg text-[11px]" style={{ background: 'rgb(var(--theme-surface-alt))' }}>160</code></div>
                <div>4. Transform → Scale: <code className="px-1.5 py-0.5 rounded-lg text-[11px]" style={{ background: 'rgb(var(--theme-surface-alt))' }}>50%</code></div>
              </div>
            </div>
          )}
        </>)}
      ...
      <SettingsCard>
        <ToggleRow
          label="Стоп анимаций при скролле"
          description="Замораживает hover-эффекты во время прокрутки. Снижает нагрузку на GPU при скролле"
          checked={freezeHoverOnScroll}
...
function ThemePreviewCard({ theme, active, onSelect, onEdit }: { theme: Theme; active: boolean; onSelect: () => void; onEdit: () => void }) {
  ...
  return (
    <div onClick={onSelect} className={cn('relative rounded-2xl overflow-hidden cursor-pointer transition-all duration-200', active ? 'scale-[1.02]' : 'hover:scale-[1.01]')}
      style={{ border: active ? '2px solid rgb(var(--theme-accent))' : '1px solid rgb(var(--theme-border)/0.4)', boxShadow: active ? '0 4px 20px rgb(var(--theme-accent)/0.25)' : undefined }}>
